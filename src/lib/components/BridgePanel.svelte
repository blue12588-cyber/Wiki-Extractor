<!--
  BridgePanel.svelte — ChatGPT 복붙 브릿지 (Slice 5b)
  ---------------------------------------------------
  AC-COPY + AC-PASTE-PARSE + AC-VALIDATE + AC-EVIDENCE-BIND + AC-IMPORT +
  AC-KOREAN-UI + AC-OFFLINE-CORE.

  하나의 후보에 대해 ChatGPT 왕복을 안내한다:

    1) 생성된 프롬프트를 보여주고 [프롬프트 복사] → 클립보드.
    2) [ChatGPT 열기] → OS 기본 브라우저로 chatgpt.com (앱은 네트워크 호출 0;
       window.open 으로 OS에 위임).
    3) 사용자가 ChatGPT JSON 응답을 textarea에 붙여넣고 [검증] →
       파싱 + 형식/필수필드/chunk_id 실재/confidence 검증.
    4) 통과 후보는 [가져오기]로 위키 후보 카드(=draft 위키 항목)로 들어간다.
       위조 chunk_id 후보는 가져오기 버튼이 비활성 + 한글 경고.

  앱 LLM 직접 호출 0: 이 컴포넌트는 fetch / invoke / LLM 클라이언트를 import 하지 않는다.
  'ChatGPT 열기'는 사용자 행위를 OS 브라우저에 위임할 뿐, 앱이 ChatGPT API를 부르지 않는다.
...
<script lang="ts">
  ...
  let { input, knownChunkIds, onimport, onclose }: Props = $props();
